...
<<<<<<< HEAD
=======
@app.route('/api/orders/<int:order_id>/assign-bot', methods=['POST'])
def assign_delivery_bot(order_id):
    ...
    try:
        ...
        # Notify customer about bot assignment
        notify_customer_status_change(order_id, 'out_for_delivery')
        
        return jsonify({'success': True, 'message': f'Delivery Bot assigned to order #{order_id}'})
    except Exception as e:
        logger.error(f"Error assigning delivery bot: {e}")
        return jsonify({'error': 'Failed to assign delivery bot'}), 500
...
def approve_driver(driver_id):
    ...
    try:
        ...
        # Notify driver via Telegram if telegram_user_id exists
        if driver.telegram_user_id:
            send_message(driver.telegram_user_id, 
                        f"🎉 Congratulations! Your driver application has been approved. You can now start accepting delivery orders.")
...
def reject_driver(driver_id):
    ...
    try:
        ...
        driver.rejection_reason = data.get('reason', 'Application rejected')
        ...
        if driver.telegram_user_id:
            send_message(driver.telegram_user_id, 
                        f"❌ Your driver application has been rejected. Reason: {driver.rejection_reason}")
        
        return jsonify({'success': True, 'message': f'Driver {driver.name} rejected'})
    except Exception as e:
...
def update_driver_location(driver_id):
    ...
    try:
        ...
        # Notify customers tracking this driver's deliveries
        active_orders = Order.query.filter_by(driver_id=driver_id, status='out_for_delivery').all()
        for order in active_orders:
            # Send location update to customer
            send_message(order.telegram_user_id, 
                        f"📍 Your delivery driver is on the way! Location updated.")
        
        return jsonify({'success': True})
    except Exception as e:
        logger.error(f"Error updating driver location: {e}")
        return jsonify({'error': 'Failed to update driver location'}), 500

@app.route('/api/orders/<int:order_id>/driver-location', methods=['GET'])
def get_driver_location_for_order(order_id):
    """Get driver's live location for specific order"""
    try:
        order = Order.query.get_or_404(order_id)
        
        if not order.driver_id:
            return jsonify({'error': 'No driver assigned to this order'}), 404
...
def get_all_driver_locations():
    ...
    try:
        ...
        return jsonify({'error': 'Failed to get driver locations'}), 500

@app.route('/api/drivers/<int:driver_id>/request-location', methods=['POST'])
def request_driver_location_update(driver_id):
    """Request location update from driver via Telegram"""
    try:
        driver = Driver.query.get_or_404(driver_id)
        
        if not driver.telegram_user_id:
            return jsonify({'error': 'Driver does not have Telegram integration'}), 400
        
        from bot_minimal import request_driver_location
        request_driver_location(driver_id)
        
        return jsonify({'success': True, 'message': f'Location request sent to {driver.name}'})
    except Exception as e:
