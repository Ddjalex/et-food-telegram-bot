...
app.use(express.static(path.join(__dirname, 'static'), { prefix: '/static' }));
...
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'templates/webapp_beu_style.html'));
});
...
app.get(['/admin', '/admin/'], requireAdmin, (req, res) => {
    const stats = {
        ...
        pending_orders: store.count('orders', { status: 'pending' }),
        total_restaurants: store.count('restaurants'),
        total_drivers: store.count('drivers'),
        active_drivers: store.count('drivers', { is_active: true, is_available: true })
    };
...
app.get('/driver-panel', (req, res) => {
    res.sendFile(path.join(__dirname, 'templates/enhanced_driver_panel.html'));
});
