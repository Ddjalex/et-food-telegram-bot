#!/usr/bin/env python3
"""
Permanent solution to ensure Flavour cafe always has 66 menu items
This script replaces the initialization data with the correct menu
"""
...
def replace_menu_permanently():
    """Replace the menu data permanently and ensure it persists"""
    ...
    with app.app_context():
        ...
        logger.info(f"🎯 Found Flavour cafe: {flavour_cafe['name']} (ID: {restaurant_id})")
        ...
        # Complete 66-item menu (same as fix_all_issues.py but embedded here)
        complete_menu = [
            # Burgers (8 items)
        ...
        for item_data in complete_menu:
            try:
                ...
                added_count += 1
                if added_count <= 5:  # Only log first 5 for brevity
        ...
        if failed_items:
            logger.warning(f"⚠️ Failed to add {len(failed_items)} items")
...
if __name__ == "__main__":
    logger.info("🚀 Starting permanent menu replacement...")
    success = replace_menu_permanently()
    if success:
        logger.info("🎉 Permanent menu fix completed successfully!")
    else:
        logger.error("❌ Permanent menu fix failed!")